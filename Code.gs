...
function generateTraceablePDF(data) {
  try {
    ...
    (data.semesters || []).forEach(sem => {
      const semCourses = data.courses[sem.id] || [];
      const graded = [];
      const planned = [];
      
      semCourses.forEach(c => {
        if (c.grade && c.grade !== '-' && c.grade !== 'I') graded.push(c);
        else planned.push(c);
      });
      ...
      if (graded.length > 0) {
        let pts = 0, cr = 0;
        graded.forEach(c => {
          const gMap = {'A':4,'B+':3.5,'B':3,'C+':2.5,'C':2,'D+':1.5,'D':1,'F':0};
          if (gMap[c.grade] !== undefined) {
            pts += gMap[c.grade] * c.credits;
            cr += c.credits;
          }
        });
        const semGPA = cr > 0 ? (pts / cr).toFixed(2) : '-';
        historySemesters.push({ name: sem.name, courses: graded, gpa: semGPA });
      }
      if (planned.length > 0) {
        planned.forEach(c => { c.semName = sem.name; plannedCourses.push(c); });
      }
    });
    
    const plannedTotalCr = plannedCourses.reduce((sum, c) => sum + (parseInt(c.credits)||0), 0);
    const gpaVal = parseFloat(data.gpax) || 0;
    let proStatusText = "สภาพวิทยฐานะ: ปกติ (Normal)";
    let proStatusColor = "#10b981"; // Emerald
    
    if (gpaVal > 0 && gpaVal < 1.75) {
      proStatusText = "สภาพวิทยฐานะ: รอพินิจ (Pro-High)";
      proStatusColor = "#ef4444";
    } else if (gpaVal > 0 && gpaVal < 2.0) {
      proStatusText = "สภาพวิทยฐานะ: รอพินิจ (Pro-Low)";
      proStatusColor = "#f59e0b";
    }
    ...
    const html = `
      ...
      <head>
        ...
        <style>
          ...
          .info-label { font-size: 10px; color: #64748b; font-weight: 800; text-transform: uppercase; margin-bottom: 4px; }
          .info-val-lg { font-size: 16px; font-weight: 800; color: #0f172a; margin-bottom: 2px; }
      ...
      <body>
        ...
        <div class="student-card">
          <div class="info-box" style="padding-right: 10px;">
            ...
            <div class="info-val-lg">${data.student.nameTh}</div>
            <div class="info-val-sm">${data.student.name}</div>
            <div class="info-val-sm" style="margin-top:5px; font-weight:600;">ID: ${data.student.id}</div>
          </div>
          <div class="info-box" style="padding-left: 10px;">
            <div class="info-label">Academic Program</div>
            <div class="info-val-sm" style="font-weight:700;">${data.student.faculty}</div>
            <div class="info-val-sm">${data.student.major}</div>
            <div class="info-val-sm" style="margin-top:5px; color:#64748b;">${data.student.degree}</div>
          </div>
        </div>

        <div class="status-banner">
          <div style="font-weight: 800; font-size: 14px; color: ${proStatusColor};">${proStatusText}</div>
          <div style="font-size: 12px; color: #475569; margin-top: 4px;">Cumulative GPAX: <strong style="color:#0f172a;">${data.gpax}</strong> | Total Credits Earned: <strong style="color:#0f172a;">${data.credits}</strong></div>
        </div>

        <h2 class="section-title">แผนการลงทะเบียนเรียน (Proposed Plan)</h2>
        ${plannedCourses.length > 0 ? `
          <table>
            ...
            <tbody>
              ...
              `).join('')}
              <tr>
                <td colspan="3" style="text-align:right; font-weight:800; border-bottom:none;">Total Requested Credits:</td>
                <td style="text-align:center; font-weight:800; color:#10b981; border-bottom:none;">${plannedTotalCr}</td>
              </tr>
            </tbody>
          </table>
        ` : `
          <div style="padding:10px; text-align:center; color:#64748b; font-size:12px; margin-bottom:20px;">ไม่มีแผนการลงทะเบียนเรียนในระบบ</div>
        `}

        <h2 class="section-title">ประวัติผลการเรียน (Grade History)</h2>
        ${historySemesters.map(sem => `
          <div style="margin-bottom: 15px;">
            <div style="font-weight:800; font-size:12px; color:#1e3a8a; margin-bottom:5px;">
              ${sem.name} <span style="font-weight:400; color:#64748b; margin-left:10px;">Term GPA: ${sem.gpa}</span>
            </div>
            <table>
              <thead>
              ...
              <tbody>
                ${sem.courses.map(c => `
                  <tr>
                    ...
                    <td>${c.nameTh}</td>
                    <td style="text-align:center;">${c.credits}</td>
                    <td style="text-align:center; font-weight:800; color:#2563eb;">${c.grade}</td>
                  </tr>
                `).join('')}
              </tbody>
            </table>
        ...
        <div class="security-block">
          <div style="font-weight:800; font-size:12px; color:#0f172a; margin-bottom:2px;">Verified Official Document</div>
          <div style="font-size:10px; color:#64748b; margin-bottom:5px;">This document is generated by the NITIPAT system and has not been tampered with.</div>
          <div style="font-family:monospace; font-size:9px; color:#3b82f6;">SIGNATURE HASH: SHA256-${signature}</div>
          <div style="font-size:9px; color:#94a3b8; margin-top:2px;">TIMESTAMP: ${data.timestamp}</div>
        </div>
      </body>
      </html>
    `;
    ...
    Logger.log("generateTraceablePDF error: " + e.toString());
    return { success: false, error: e.toString() };
  }
}
