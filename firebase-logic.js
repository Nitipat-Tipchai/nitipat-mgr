...
async function loadAll() {
  ...
  try {
    ...
    if (secSnap.exists()) {
      state.securitySettings = secSnap.data();
    ...
    if (calSnap.exists()) {
      state.calendarSettings = calSnap.data();
    }
...
window.autoArchiveCourses = autoArchiveCourses;

window.uploadToFirebaseStorage = async function(file, path) {
  if (!window.getStorage || !window.app) {
    throw new Error('Firebase Storage is not initialized');
  }
  const storage = window.getStorage(window.app);
  const storageRef = window.ref(storage, path);
  const snapshot = await window.uploadBytes(storageRef, file);
  const downloadURL = await window.getDownloadURL(snapshot.ref);
  return downloadURL;
};

window.deleteFromFirebaseStorage = async function(pathOrUrl) {
  if (!window.getStorage || !window.app) {
    throw new Error('Firebase Storage is not initialized');
  }
  const storage = window.getStorage(window.app);
  ...
  if (pathOrUrl.startsWith('http')) {
    storageRef = window.ref(storage, pathOrUrl);
  } else {
    storageRef = window.ref(storage, pathOrUrl);
  }
