...
function renderSettings() {
  ...
  <div class="settings-card" style="margin-bottom:15px;">
    ...
    <!-- OKMD AI Playground Settings -->
    <div class="glass-card settings-block" style="border: 1.5px solid rgba(99, 102, 241, 0.3); background: rgba(99, 102, 241, 0.04); margin-bottom:15px;">
      <div class="setting-title" style="color: var(--c-indigo); font-weight:700;">🤖 OKMD AI Playground Settings</div>
      <div style="font-size:11px; color:var(--c-muted); margin-bottom:12px;">เชื่อมต่อ API เพื่อใช้งาน AI Quick-Add และผู้ช่วย AI Assistant</div>
      
      <div class="setting-row" style="flex-direction:column; align-items:flex-start; gap:8px; margin-bottom:10px;">
        <label style="font-size:12px; font-weight:700;">OKMD API Key</label>
        <div style="display:flex; gap:8px; width:100%;">
          <input type="password" class="glass-input sm" id="okmdAiKeyInput" placeholder="sk_cz...xxxx" value="${localStorage.getItem('okmd_ai_api_key') || ''}" style="flex:1;">
          <button class="btn-glass-primary sm" onclick="saveOkmdAiSettings()">💾 บันทึก Key</button>
        </div>
      </div>

      <div class="setting-row" style="flex-direction:column; align-items:flex-start; gap:8px;">
        <label style="font-size:12px; font-weight:700;">เลือกโมเดล AI (Model)</label>
        <select id="okmdAiModelSelect" class="glass-input sm" style="width:100%; font-weight:600;" onchange="saveOkmdAiSettings()">
          ${[
            'gemini-2.5-flash-lite', 'gemini-3.1-flash-lite', 'gemini-3.5-flash', 'gemini-3.1-pro-preview',
            'gpt-5.4-mini', 'gpt-5.4', 'deepseek-v4-flash', 'deepseek-v4-pro',
            'claude-sonnet-4.6', 'qwen3.6-flash', 'llama-4-scout'
          ].map(m => `<option value="${m}" ${ (localStorage.getItem('okmd_ai_model') || 'gemini-2.5-flash-lite') === m ? 'selected' : '' }>${m}</option>`).join('')}
        </select>
      </div>
    </div>
...
window.saveOkmdAiSettings = function() {
  const keyInput = document.getElementById('okmdAiKeyInput');
  const modelSelect = document.getElementById('okmdAiModelSelect');

  if (keyInput) {
    localStorage.setItem('okmd_ai_api_key', keyInput.value.trim());
  }
  if (modelSelect) {
    localStorage.setItem('okmd_ai_model', modelSelect.value);
  }

  showToast('💾 บันทึกตั้งค่า OKMD AI สำเร็จแล้ว!');
};
