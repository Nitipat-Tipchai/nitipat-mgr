...
function renderCurriculumMap() {
  ...
  // Roadmap Data
  const sections = [
    { label: '📖 หมวดวิชาศึกษาทั่วไป (30 cr)', courses: typeof COURSE_DB !== 'undefined' ? COURSE_DB.general : [], target: 30 },
    { label: '🔬 พื้นฐานทางวิทยาศาสตร์ (21 cr)', courses: typeof COURSE_DB !== 'undefined' ? COURSE_DB.science : [], target: 21 },
    { label: '⚙️ พื้นฐานทางวิศวกรรม (27 cr)', courses: typeof COURSE_DB !== 'undefined' ? COURSE_DB.engineering_basic : [], target: 27 },
    { label: '🏗 วิชาบังคับทางวิศวกรรม (37 cr)', courses: typeof COURSE_DB !== 'undefined' ? COURSE_DB.core : [], target: 37 },
    { label: '🔧 วิชาเลือกทางวิศวกรรม (16 cr)', courses: typeof COURSE_DB !== 'undefined' ? COURSE_DB.elective : [], target: 16 },
  ];
  ...
  if (state.customRoadmapCourses) {
    state.customRoadmapCourses.forEach(cc => {
      ...
      if (sec && !sec.courses.find(c => c.code === cc.code)) {
        sec.courses.push({ code: cc.code, name: cc.name, credits: cc.cr });
      }
    });
  }
        ...
        return \`<div class="glass-card roadmap-section">
          ...
          <div class="rm-courses">
            \${sec.courses.map(c => {
              ...
              const prereqOk = typeof checkPrereqs === 'function' ? checkPrereqs(c.code) : {ok: true};
