...
function handleILMUpload(input) {
  ...
  showToast("กำลังอัปโหลดไฟล์ไปยัง Google Drive... อาจใช้เวลาสักครู่");
  ...
  reader.onload = async function(e) {
    ...
    try {
      const driveRes = await new Promise((res, rej) => {
        google.script.run.withSuccessHandler(res).withFailureHandler(rej).uploadVaultFileToDrive(dataUrl, file.name);
      });
      
      if (!driveRes || !driveRes.success) {
        showToast("❌ เกิดข้อผิดพลาดในการอัปโหลดไฟล์: " + (driveRes ? driveRes.error : "Unknown Error"));
        return;
      }
      ...
      showToast("✅ อัปโหลดไฟล์เสร็จสิ้น!");
      renderILMExplorer();
    } catch(err) {
      console.error(err);
      showToast("❌ ไม่สามารถติดต่อ Google Drive ได้");
    }
  };
    ...
    <div style="text-align:left; font-family:Sarabun, sans-serif;">
      ...
      <div class="i-fg">
        ...
        <div style="display:flex; align-items:center; gap:5px; background:var(--bg); padding:2px 8px; border-radius:10px; border:1px solid var(--i-border);">
          <span style="font-size:0.8rem; color:var(--text-muted); font-family:monospace;">share.html?doc=</span>
      ...
      <div style="margin-top:20px; background:rgba(30, 58, 138, 0.04); border:1px solid var(--i-primary-light); padding:12px; border-radius:12px;">
        ...
        <div id="share-url-preview" style="font-family:monospace; font-size:0.75rem; color:var(--text); word-break:break-all; font-weight:bold;">
          ${window.location.origin}/share.html?doc=${initialSlug}
        </div>
      </div>
...
function updateSharePreviewURL() {
  ...
  if (preview) {
    preview.innerText = `${window.location.origin}/share.html?doc=${slug || 'resume'}`;
  }
}
...
function saveILMFileShareSettings(fileId) {
  ...
  const finalURL = `${window.location.origin}/share.html?doc=${slug}`;
