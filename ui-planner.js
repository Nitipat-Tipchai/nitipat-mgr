...
function renderPlanner() {
  ...
  return `<div class="page-wrap" style="background: linear-gradient(135deg, #f8fafc, #eff6ff); min-height: 100vh; padding: 20px; border-radius: 24px;">
    ...
    <!-- AI Natural Language Quick Input Bar -->
    <div class="glass-card" style="padding:18px 22px; margin-bottom:20px; border-radius: 20px; background: linear-gradient(135deg, rgba(99,102,241,0.08), rgba(236,72,153,0.05)); backdrop-filter: blur(20px); border: 1px solid rgba(99,102,241,0.2); box-shadow: 0 10px 25px rgba(99,102,241,0.05);">
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
        <div style="font-weight:800; font-size:14px; color:var(--c-indigo); display:flex; align-items:center; gap:6px;">
          <span>✨</span> AI Quick-Add (สั่งเพิ่มงานด้วยภาษาธรรมชาติ)
        </div>
        <span style="font-size:11px; color:#64748b; font-weight:600;">ลองพิมพ์: "ส่งการบ้านฟิสิกส์ 15 ส.ค. 5 โมงเย็น" หรือ "สอบแคลคูลัส 2 พรุ่งนี้ 9 โมงเช้า"</span>
      </div>
      <div style="display:flex; gap:10px;">
        <input type="text" id="aiPlannerInput" class="glass-input" placeholder="✨ พิมพ์กิจกรรม เช่น 'สอบแคลคูลัส 2 วันที่ 15 ส.ค. 9 โมงเช้า ห้อง 4201'..." style="width:100%; border-radius:14px; padding:12px 16px; font-weight:600;" onkeydown="if(event.key==='Enter') executeAiPlannerAdd()">
        <button class="btn-glass-primary" id="aiPlannerAddBtn" onclick="executeAiPlannerAdd()" style="border-radius:14px; white-space:nowrap; padding:0 20px; font-weight:800; background:linear-gradient(135deg, #6366f1, #8b5cf6);">🤖 วิเคราะห์ & เพิ่มงาน</button>
      </div>
    </div>
...
window.executeAiPlannerAdd = async function() {
  const input = document.getElementById('aiPlannerInput');
  const btn = document.getElementById('aiPlannerAddBtn');
  if (!input) return;

  const promptText = input.value.trim();
  if (!promptText) return showToast('กรุณากรอกข้อความกิจกรรมที่ต้องการเพิ่มครับ', 'info');
  ...
  try {
    showToast('🤖 AI กำลังถอดรหัสข้อมูล...', 'info');
    const parsed = await parseTaskWithAI(promptText);

    if (!parsed || !parsed.title) {
      throw new Error('ไม่พบข้อมูลชื่องานที่ชัดเจน');
    }
    ...
    const newTask = {
      ...
      due: parsed.due || new Date().toISOString().split('T')[0],
      startTime: parsed.startTime || null,
      endTime: parsed.endTime || null,
      kanbanStage: parsed.kanbanStage || 'To Do',
      done: false,
      note: parsed.note || promptText
    };

    if (!state.plannerTasks) state.plannerTasks = [];
    state.plannerTasks.push(newTask);
    savePlannerTasksState();

    input.value = '';
    showToast(`✅ สั่งงาน AI สำเร็จ: ${newTask.title}`, 'success');

    if (state.calendarConfig && state.calendarConfig.syncEnabled && typeof forceSyncCalendar === 'function') {
      forceSyncCalendar(true);
    }

    render();
  } catch (err) {
    console.error('AI Add Error:', err);
    showToast(`❌ ${err.message || 'เกิดข้อผิดพลาดในการวิเคราะห์ AI'}`, 'err');
  } finally {
    if (btn) {
      btn.disabled = false;
